...
// A balance item unchanged for 4 quarters (the last-FY impairments: 8) is stale: dropped,
// not carried, so an abandoned field never passes as current. nq counts store quarters.
var int nq = 0
...
K(int i) =>
    float x = known.get(i)
    BAL.includes(i) and nq - lastq.get(i) >= (i == 15 ? 8 : 4) ? na : x
...
if Q.rows() > 0 and (changed or last_q == bar_index)
    ...
    // EBITDA backup needs amortisation too (depreciation alone understates it): no
    // amortisation figure -> depreciation only, rated as a ratio.
    float em = pct(K(9))
    [ed, ced] = pick(K(6), 1, not na(am) ? K(7) + dd + am : na, 2, rev > 0 ? em * rev : na, 3)
    if na(ed) and not na(K(7) + dd)
        ed := K(7) + dd
        ced := 3
    put(2, ed, ced)

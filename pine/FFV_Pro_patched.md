...
i_use_rkv = input.bool(true, '💎 Value-trap filter (justified P/B)', group = group_calc, tooltip = 'Justified P/B = (ROE - g) / (cost of equity - g) on the 5-year average ROE (Wilcox 1984; Ohlson 1995). A value trap is a P/B under 1 that is no higher than that: cheap on book, but the returns do not support even that price. Flags it, takes 0.2 off our confidence and puts a soft ceiling on the scorecard\'s Value pillar (over 50 it keeps a quarter of its excess). Also drops quarters with ROE under the 10Y + 5% from the P/B history average.')
...
// terminal growth, held 2pp under the cost of equity. Trap: P/B under 1 and no higher than that.
...
if i_use_rkv and bvps_ttm > 0 and cost_of_equity > 0
    ...
    is_value_trap := current_pb_val < 1 and current_pb_val >= jpb
// ==============================================================
...
if barstate.islast and i_show_street
    ...
    float oc_flags = (is_value_trap ? 0.2 : 0.0) + (i_useBeneishCheck and F_manip ? 0.2 : 0.0) + (altman_z < z_safe_cut ? 0.2 : 0.0) + (F_suspect ? 0.2 : 0.0)
...
if barstate.islast
    ...
    if is_value_trap
        fl.push('Value trap')
        flags_tt += str.format(FL.tx(43), current_pb_val, jpb_roe * 100, cost_of_equity * 100)
    if wacc_flag != 'Normal'
        fl.push('WACC')
...
if barstate.islast
    ...
    CARD := FL.f_card(array.from(F_gp / F_assets, F_ocf / F_assets, D.roe, F_ni_c / F_assets, (F_ni_c - F_ocf) / F_assets, F_gp / F_rev, float(na), float(na), float(na), float(na), float(na),
         ...
         F_debt / F_assets < 0.02 ? 99.0 : F_interest > 0 ? F_ebit / F_interest : float(na), float(na), float(na), float(na), close > 0 and F_t_oe > 0 ? F_oe_ps / close - cost_of_equity : float(na),
         ...
         FL.f_oscore(F_assets / fx_rate, F_assets, F_tl, F_ca, F_cl, F_ni_c, F_ni_1y, F_da, year),
         ...
         i_useBeneishCheck and F_manip and math.min(eng_t.get(19), eng_t.get(5), eng_t.get(6), eng_t.get(0)) >= 2, is_value_trap,
    ...
    CARD_INFO.concat(array.from('1', 'Z+M matrix', 'Z ' + (na(altman_z) ? '-' : str.tostring(altman_z, '#.#')), 'information', HV_zm_txt, HV_zm_tt,
         ...
         is_value_trap ? 'Value trap' : na(jpb) or na(current_pb_val) ? 'information' : current_pb_val < jpb ? 'Below justified' : 'Above justified', na(jpb) or na(current_pb_val) ? 'Justified P/B needs a positive book value and 3 yearly ROE readings (the filter setting on).' : str.format(FL.tx(37), current_pb_val, jpb, jpb_roe * 100, cost_of_equity * 100, jpb_g * 100)))
// ---------- summary card ----------
f_tbl_head() =>

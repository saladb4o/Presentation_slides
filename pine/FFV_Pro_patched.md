...
// than max(40%, 1 / families present) of a blend. 0 Composite | 1 earnings / returns |
// 2 sales | 3 cash flow | 4 book | 5 EBIT(DA) | 6 dividends. (P/AFFO reads the FCF
// figure: no AFFO field exists, so it is a cash-flow model like P/FCF.)
var array<int> FAM = array.from(0, 1, 2, 3, 4, 4, 5, 3, 3, 3, 3, 3, 3, 3, 1, 6, 3, 1, 1, 1, 2, 5, 3)
...
f_format_period(PeriodStats stats) =>
    ...
    if p.n == 0
        ...
        float md = nz(p.med_ret) * 100
        string main_txt = str.tostring(md, "#.0") + "% (" + str.tostring(p.wr, "#.0") + "%) n=" + str.tostring(p.n)
        string tt = "Trades: " + str.tostring(p.n) +
             ...
             "\n\nHeadline = median trade return after fees, dividends accrued. VACAGR annualizes each trade with a 1-month floor, so short holds dominate it."
...
f_fill_robust_row(int row, string name, ModelStats m, ModelStats b) =>
    ...
    for w = 1 to 3
        ...
        // Median trade return (%), the same headline as the matrix.
        float q = nz(f_period_metrics(f_ps(b, w)).med_ret) * 100
        float pv = p.med_ret * 100
        array.push(v, pv)
        array.push(n, p.n)
        edge_sum += nz(pv) - q
...
f_bt_table() =>
    ...
    for c = 0 to array.size(hd) - 1
        string tt = c == 0 ? 'The inputs were chosen after seeing this whole history, so for the SETTINGS every period is in-sample. Read P2/P3 as a stability check, not proof of an edge.' : v_mx ? 'Median trade % (Win%) n=trades' : v_rb ? (c == 6 ? 'Average median-trade edge over the always-in baseline across the three periods.' : '') : c == 2 ? 'Discount-to-return correlation: does a bigger discount predict a bigger return?' : c == 7 ? 'Per-trade maximum adverse excursion, NOT portfolio drawdown.' : ''

...
// peq_1y: parent equity a year earlier | fcfm_med: median FCFF margin | g1: stage-1 growth |
...
// operating profitability, FCFF margin | CLOSE: price, the value shown, then one column per row
...
int Q_PEQ = 53, int Q_ROE = 54, int Q_OP = 55, int Q_FCFM = 56, int Q_PX = 57, int Q_FFV = 58, int Q_FV = 59
...
if CK.dirty
    ...
    // quarter is dirty. Loss quarters stay in: dropping them biased the medians (ROE, FCFF
    // margin) upward.
    float peq = f_peq(F, MK)
    ...
    ST.write(Q_FCFM, F.rev > 0 ? math.max(math.min(F.fcff / F.rev, 1.0), -1.0) : na)
    ...
    H := Hist.new(median_roe, peq_1y, f_median(ST.lastn(Q_FCFM, 20, 0)), g1, earnings_base, not na(g1) and not cape_on ? earnings_base * (1 + g1) : na, est_ebitda_fwd, fwd_growth_leg, fwd_growth_src, cape_on)
// =====================================================================
...
// Reverse DCF input: the median unlevered FCF margin on today's revenue (no history -> N/A).
float dcf_input_fcf_total_rev = F.rev * H.fcfm_med

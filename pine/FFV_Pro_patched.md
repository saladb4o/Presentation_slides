...
// ==============================================================
// Snapshot data (same value on every bar). Display only.
f_clamp01(float x) =>
...
f_st_growth(float pv) =>
    not na(pv) and pv > 0 ? f_calculate_reverse_dcf(pv, dcf_input_fcf_total_rev, shares_out_latest, math.max(final_discount_rate, 0.01), final_terminal_growth, 10) : na
...
float st_pe_rank_hi = f_pct_rank(M_PE.hist, st_pe_hi)
float st_pe_rank_md = f_pct_rank(M_PE.hist, st_pe_md)
...
if barstate.islast
    ...
    if i_show_street
        ...
        f_cell(T, 0, row_idx, 'Score', color_text, color_bg, i_textSize)
        f_cell(T, 1, row_idx, f_ctxt(our_conf), color.white, f_ccol(our_conf), i_textSize)
        f_cell(T, 2, row_idx, f_ctxt(st_conf), color.white, f_ccol(st_conf), i_textSize)
        ...
        for j = 0 to 3
            table.cell(T, 0, row_idx, '  ' + array.get(cl, j), text_color = color_text, bgcolor = color_bg, text_size = i_textSize, tooltip = array.get(ctt, j))
            f_cell(T, 1, row_idx, f_stxt(array.get(co, j)), color_text, color_bg, i_textSize)
            f_cell(T, 2, row_idx, f_stxt(array.get(cs, j)), color_text, color_bg, i_textSize)
            f_cell(T, 3, row_idx, '', color_text, color_bg, i_textSize)
            row_idx += 1
        if not na(cw_fv)
            ...
            f_cell(T, 3, row_idx, (close / cw_fv - 1 > 0 ? '+' : '') + str.tostring((close / cw_fv - 1) * 100, '#') + '% vs price', color_text, color_bg, i_textSize)
            row_idx += 1
    // ================= DIAGNOSTICS ==================
